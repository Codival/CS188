...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if not legalActions:
            return 0.0
        qValuesList = [self.getQValue(state, action) for action in legalActions]
    ...
    def update(self, state, action, nextState, reward):
        ...
        nextQValue = [self.getQValue(nextState,action) for action in self.getLegalActions(nextState)]
        if not nextQValue:
            maxNextQValue = 0
        else:
            maxNextQValue = max(nextQValue)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        diff = (reward + (self.discount * maxNextQValue)) - currentQValue

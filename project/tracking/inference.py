...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        lower,rand, upper = 0, random.random(), 0
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.allPositions:
            self.beliefs[p] = self.beliefs[p]*self.getObservationProb(observation, gameState.getPacmanPosition(), p, self.getJailPosition())
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.legalPositions:
            update[p] = 0.0
        for p in self.allPositions:
            prob = self.getPositionDistribution(gameState, p)
            for p2 in self.allPositions:
                update[p2] += prob[p2]*self.beliefs[p]
        self.beliefs = update
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for _ in range(num//length):
            self.particles += self.legalPositions
        if num%length != 0:
            self.particles += self.legalPositions[0:(num%length)]
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        ghostPerm = self.legalPositions
        for _ in range(self.numGhosts-1):
            ghostPerm = itertools.product(ghostPerm, self.legalPositions)
        ghostPermList = list(ghostPerm)
        random.shuffle(ghostPermList)
        length = len(ghostPermList)
        num = self.numParticles
        for _ in range(num//length):
            self.particles += ghostPermList
        if num%length != 0:
            self.particles += ghostPermList[0:(num%length)]
